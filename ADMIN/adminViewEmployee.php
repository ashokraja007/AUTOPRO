...
$qry = "SELECT `login`.*,`employee`.* FROM `login`,`employee` WHERE `login`.`reg_id`=`employee`.`emp_id` AND `login`.`usertype`='Employee'";
$result = mysqli_query($conn, $qry);
if (mysqli_num_rows($result) < 1) {
?>
    <center>
        <h1 id="nodata" class="m-3">No Employees Registered</h1>
    </center>
<?php
