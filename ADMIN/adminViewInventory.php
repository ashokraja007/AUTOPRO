...
$qry = "SELECT `inventory`.*,`product`.* FROM `inventory`,`product` WHERE `inventory`.`pdt_id`=`product`.`pdt_id` AND `inventory`.`status`='Paid'";
            ...
            <tbody id="tableBody">
                ...
                ?>
                    <tr id="row{{ forloop.counter }}" style="text-align: center;">
                        ...
                        ?>
                            <td>
                                <h5 style="color: red;font-weight: bolder;font-size: 20px;">Low Stock <img src="../assets/images/warning.svg" height="25px" width="25px" alt="Hello"></h5><br>
                                <!-- <a href="adminRequestProduct.php?pid=<?php echo $row['pdt_id'] ?>&sid=<?php echo $row['sid'] ?>" class="btn btn-custom-01"> Request Now</a> -->
                            </td>
                        <?php } ?>
                    </tr>
                <?php
