...
$qry = "SELECT `login`.*,`supplier`.* FROM `login`,`supplier` WHERE `login`.`reg_id`=`supplier`.`sid` AND `login`.`usertype`='Supplier'";
$result = mysqli_query($conn, $qry);
if (mysqli_num_rows($result) < 1) {
?>
    <center>
        <h1 id="nodata" class="m-3">No Suppliers Registered</h1>
    </center>
<?php
                ...
                ?>
                    <tr id="row{{ forloop.counter }}" style="text-align: center;">
                        ...
                        <td id="statusPay">
                            ...
                            ?>
                                <a class="btn btn-outline-success" href="manageSupplier.php?id=<?php echo $row['reg_id'] ?>&status=Approved">Approve</a>
                                <a class="btn btn-outline-danger" href="manageSupplier.php?id=<?php echo $row['reg_id'] ?>&status=Rejected">Reject</a>
                            <?php
